...
class RAGFlowPdfParser:
    def __init__(self):
        ...
        try:
            model_dir = os.path.join(
                os.path.dirname(__file__),  # Adjust path
                "deepdoc"
            )
    ...
    def _updown_concat_features(self, up, down):
        ...
        fea = [
            ...
            True if re.search(r"([。？！；!?;+)）]|[a-z]\.)$",
                              up["text"]) else False,
            True if re.search(r"[，：‘“、0-9（+-]$", up["text"]) else False,
            True if re.search(
                r"(^.?[/,?;:\]，。；：’”？！》】）-])", down["text"]) else False,
            ...
            up["text"].strip()[-2:] == down["text"].strip()[-2:] if len(up["text"].strip()
                                                                        ) > 1 and len(down["text"].strip()) > 1 else False,
            up["x0"] > down["x1"],
    ...
    def _table_transformer_job(self, ZM):
        ...
        for p, tbls in enumerate(self.page_layout):  # for page
            ...
            for tb in tbls:  # for table
                left, top, right, bott = tb["x0"] - MARGIN, tb["top"] - \
                    MARGIN, tb["x1"] + MARGIN, tb["bottom"] + MARGIN
                left *= ZM
                top *= ZM
                right *= ZM
                bott *= ZM
    ...
    def _naive_vertical_merge(self):
        bxs = Recognizer.sort_Y_firstly(
            self.boxes, np.median(self.mean_height) / 3)
        i = 0
        while i + 1 < len(bxs):
            b = bxs[i]
            b_ = bxs[i + 1]
    ...
    def _filter_forpages(self):
        ...
        while i < len(self.boxes):
            ...
            findit = True
            eng = re.match(
                r"[0-9a-zA-Z :'.-]{5,}", self.boxes[i]["text"].strip())
            self.boxes.pop(i)
            if i >= len(self.boxes):
                break
    ...
    def _extract_table_figure(self, need_image, ZM, return_html, need_position):
            ...
            def nearest(tbls):
                ...
                for k, bxs in tbls.items():
                    for b in bxs:
                        ...
                        x_dis = self._x_dis(
                            c, b) if not x_overlapped(c, b) else 0
        ...
        def cropout(bxs, ltype, poss):
            ...
            if len(pn) < 2:
                ...
                if ii is not None:
                    ...
                    logging.warn(
                        f"Missing layout match: {pn + 1},%s" % (bxs[0].get("layoutno", "")))
        ...
        for k, bxs in tables.items():
            ...
            poss = []
            res.append((cropout(bxs, "table", poss), self.tbl_det.construct_table(
    ...
    def __filterout_scraps(self, boxes, ZM):
        ...
        while boxes:
            ...
            mj = self.proj_match(boxes[0]["text"]) or boxes[0].get(
                "layout_type", "") == "title"
            ...
            def dfs(line, st):
                ...
                mmj = self.proj_match(line["text"]) or line.get(
                    "layout_type", "") == "title"
            ...
            if mj or mw / pw >= 0.35 or mw > 200:
                ...
                logging.debug(
                    "REMOVED: " + "<<".join([c["text"] for c in lines]))
    ...
    def __images__(self, fnm, zoomin=3, page_from=0, page_to=299, callback=None):
        ...
        try:
            ...
            self.page_images = [p.to_image(
                resolution=72 * zoomin).annotated for i, p in enumerate(self.pdf.pages[page_from:page_to])]
            self.page_chars = [[{**c, 'top': c['top'], 'bottom': c['bottom']} for c in page.dedupe_chars(
        ...
        self.is_english = [re.search(r"[a-zA-Z0-9,/¸;:'\[\]\(\)!@#$%^&*\"?<>._-]{30,}", "".join(
            random.choices([c["text"] for c in self.page_chars[i]], k=min(100, len(self.page_chars[i]))))) for i in range(len(self.page_chars))]
    ...
    def crop(self, text, ZM=3, need_position=False):
        ...
        for ii, (pns, left, right, top, bottom) in enumerate(poss):
            ...
            imgs.append(self.page_images[pns[0]].crop((left * ZM, top * ZM,
                                                      right * ZM, min(bottom, self.page_images[pns[0]].size[1]))))
            if 0 < ii < len(poss) - 1:
                positions.append((pns[0] + self.page_from, left, right,
                                 top, min(bottom, self.page_images[pns[0]].size[1]) / ZM))
            bottom -= self.page_images[pns[0]].size[1]
            for pn in pns[1:]:
                imgs.append(self.page_images[pn].crop((left * ZM, 0,
                                                      right * ZM, min(bottom, self.page_images[pn].size[1]))))
                if 0 < ii < len(poss) - 1:
                    positions.append((pn + self.page_from, left, right,
                                     0, min(bottom, self.page_images[pn].size[1]) / ZM))
...
class PlainParser(object):
    def __call__(self, filename, from_page=0, to_page=100000, **kwargs):
        ...
        lines = []
        try:
            self.pdf = pdf2_read(filename if isinstance(
