...
class Recognizer(object):
    ...
    def postprocess(self, boxes, inputs, thr):
        ...
        boxes = boxes[:, :4]
        input_shape = np.array([inputs["scale_factor"][0], inputs["scale_factor"]
                               [1], inputs["scale_factor"][0], inputs["scale_factor"][1]])
    ...
    def __call__(self, image_list, thr=0.7, batch_size=16):
        ...
        for i in range(batch_loop_cnt):
            ...
            print("preprocess")
            for ins in inputs:
                bb = self.postprocess(self.ort_sess.run(
                    None, {k: v for k, v in ins.items() if k in self.input_names})[0], ins, thr)
                res.append(bb)
